...
class LoginController extends Controller
{
    ...
    public function Only()
    {
        ...
        $tel = Input::get('tel');
        //print_r($tel);die;
        $user = DB::table('users')->where('user', '=',$tel)->get();
        if($user){
            return 1;
        }else{
            return 0;
    ...
    {   //echo Crypt::encrypt('k123456');die;
        ...
        //print_r($username);
       // print_r($pwd);die;
        $arr = DB::table('users')->where('user',$username)->first();
        //print_r($arr);die;
        if($arr){
            if($pwd !=Crypt::decrypt($arr->pwd)){
    ...
    public function Weibo()
    {
        ...
        //先查询用户是否存在本网站中
        $res = DB::table('users')->where('auth_id','=',$userinfo['id'])->get();
        if($res){
            //有的话直接登陆  存session
            $session = new Session;
    ...
    public function Qqlogin()
    {
       ...
       // print_r($token);die;
        ...
        //查一下此用户是否在本网战使用qq登陆过
        $res = DB::table('users')->where('auth_id','=',$open_id['openid'])->get();
        if($res){
            //如果有直接存session  进首页
            $session = new Session;
...
public function register(){
    ...
    public function adds()
    {
        ...
        // if($re['tel']==$user['user']){
        //       echo "<script>alert('手机号已存在');localhost.href='{{url('regist')}}'</script>";
        // }
        if($re){
            $session = new Session();
